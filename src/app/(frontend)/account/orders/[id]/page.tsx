    ...
    <div className="space-y-32">
      <header className="flex items-start justify-between">
        <div>
          ...
          <div className="text-stone text-[13px] mt-4">
            {new Date(order.createdAt).toLocaleString()}
          </div>
          {order.invoiceNumber && (
            <div className="text-stone text-[12px] mt-2">Invoice: {order.invoiceNumber}</div>
          )}
        </div>
        <div className="flex flex-col items-end gap-8">
          ...
          <div className="flex gap-8">
            <Link
              ...
              target="_blank"
              className="font-mono text-[11px] uppercase tracking-widest underline text-stone hover:text-ink"
            >
              View Invoice
            </Link>
            <a
          ...
          {order.items?.map((item: any, i: number) => (
            <div key={i} className="flex justify-between border-b border-stone/20 pb-12">
              <div>
                ...
                <div className="font-mono text-[11px] uppercase text-stone">
          ...
          <ol className="space-y-8 text-[13px]">
            {order.timeline.map((e: any, i: number) => (
              <li key={i} className="flex gap-12">
                <span className="font-mono text-stone w-[140px]">
